...
function potion_npc:on_interaction()
  ...
  if first_empty_bottle == nil then
    game:start_dialog("kakariko.potion_house.no_bottle")
  else
    game:start_dialog("kakariko.potion_house.offer", price, function(answer)
      if answer == 3 then
        if game:get_money() < price then
          game:start_dialog("not_enough_money")
        else
          game:start_dialog("kakariko.potion_house.yes", function()
